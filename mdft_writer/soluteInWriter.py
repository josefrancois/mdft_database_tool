...
class SoluteInWriter:
	...
	def write(self, folder):
		with open(os.path.join(folder, "solute.in"), 'w') as solutein:
			solutein.write(self.molecule.getName() + '\t' + self.molecule.getCalcDg() +  '\t' + self.molecule.getExpDg() + "\t#systeme  DGsolv_calc (kJ/mol)  DGsolv_expt (kJ/mol)" + '\n')
			#solutein.write("solute" + '\n')
			solutein.write(str(self.molecule.getNumberOfAtoms()) + '\n')
			solutein.write("{0:5s}{1:10s}{2:15s}{3:20s}{4:10s}{5:10s}{6:9s}{7}\n"\
			.format("#","charge", "sigma(Ang)", "epsilon(kJ/mol)", "x", "y", "z", "Zatomic"))
			for atom in self.molecule.getListAtoms():
				solutein.write("{0:10s}{1:10.6f}{2:10.5f}{3:10.5f}{4:10.3f}{5:10.3f}{6:10.3f}{7:10d}\n"\
