...
class SoluteInWriter:
    ...
    def write(self, folder):
        with open(os.path.join(folder, "solute.in"), 'w') as solutein:
            solutein.write(self.molecule.getName() + '\t' + str(self.molecule.getData()['calc']*4.187) +  '\t' + str(self.molecule.getData()['expt']*4.187) + "\t#systeme  DGsolv_calc (kJ/mol)  DGsolv_expt (kJ/mol)" + '\n')
